...
Скопируйте папку `creative-director/` в директорию скиллов OpenClaw:

```bash
# Клонируйте
git clone https://github.com/smixs/creative-director-skill.git

# Скопируйте в скиллы OpenClaw
cp -r creative-director-skill/creative-director ~/.openclaw/workspace/skills/
```

Скилл активируется автоматически, когда вы просите Claude сгенерировать креативные концепции, побрейнштормить идеи кампании, разработать Big Idea, оценить креативную работу или найти потребительский инсайт.

### Claude Projects / другие платформы

Добавьте файлы в базу знаний проекта. Начните с `SKILL.md` - он ссылается на другие файлы через `[[wikilinks]]`, которые можно подключить вручную или загрузить все файлы сразу.
